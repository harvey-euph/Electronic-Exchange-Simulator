...
void SQLiteClientDatabase::init_tables() {
    ...
    execute_sql(R"(
        CREATE TABLE IF NOT EXISTS client_log_offsets (
            client_id INTEGER,
            o_seq_num INTEGER,
            log_offset INTEGER,
...
std::vector<OrderResponseT> SQLiteClientDatabase::getResponsesSince(uint32_t client_id, uint64_t ack_seq_num) {
    std::lock_guard<std::mutex> lock(mutex_);
    std::vector<OrderResponseT> result;
    
    sqlite3_stmt* stmt;
    const char* sql = "SELECT o_seq_num, log_offset FROM client_log_offsets WHERE client_id = ? AND o_seq_num > ? ORDER BY o_seq_num ASC;";
    if (sqlite3_prepare_v2(db_, sql, -1, &stmt, nullptr) == SQLITE_OK) {
        sqlite3_bind_int(stmt, 1, client_id);
        sqlite3_bind_int64(stmt, 2, ack_seq_num);
        mmaplog::MmapReader reader(EXECUTION_JOURNAL_DIR);
        while (sqlite3_step(stmt) == SQLITE_ROW) {
            uint64_t offset = sqlite3_column_int64(stmt, 1);
...
void SQLiteClientDatabase::update_on_execution(const OrderResponseT* resp, uint64_t log_offset) {
    std::lock_guard<std::mutex> lock(mutex_);
    ...
    const char* sql_seq = "INSERT INTO clients (client_id, username, o_seq_num) VALUES (?, ?, 1) ON CONFLICT(client_id) DO UPDATE SET o_seq_num = clients.o_seq_num + 1 RETURNING o_seq_num;";
    if (sqlite3_prepare_v2(db_, sql_seq, -1, &stmt_seq, nullptr) == SQLITE_OK) {
        sqlite3_bind_int(stmt_seq, 1, client_id);
        std::string username = "client_" + std::to_string(client_id);
        sqlite3_bind_text(stmt_seq, 2, username.c_str(), -1, SQLITE_TRANSIENT);
        if (sqlite3_step(stmt_seq) == SQLITE_ROW) {
            msg_seq_num = sqlite3_column_int64(stmt_seq, 0);
        }
    ...
    if (check_exec(resp->exec_type, EXEC_ALIVE)) {
        ...
        if (sqlite3_prepare_v2(db_, sql, -1, &stmt, nullptr) == SQLITE_OK) {
            ...
            sqlite3_bind_int(stmt, 4, static_cast<int16_t>(resp->side));
            sqlite3_bind_int64(stmt, 5, resp->p);
        ...
        uint64_t combined_order_id = (static_cast<uint64_t>(client_id) << 32) | resp->order_id;
        sqlite3_stmt* stmt;
    ...
    const char* sql_log = "INSERT INTO client_log_offsets (client_id, o_seq_num, log_offset) VALUES (?, ?, ?);";
    if (sqlite3_prepare_v2(db_, sql_log, -1, &stmt_log, nullptr) == SQLITE_OK) {
        sqlite3_bind_int(stmt_log, 1, client_id);
        sqlite3_bind_int64(stmt_log, 2, msg_seq_num);
        sqlite3_bind_int64(stmt_log, 3, log_offset);
        sqlite3_step(stmt_log);
        sqlite3_finalize(stmt_log);
    }

    sqlite3_stmt* stmt_state;
    const char* sql_state = "INSERT INTO system_state (key, value) VALUES ('log_offset', ?) ON CONFLICT(key) DO UPDATE SET value = excluded.value;";
    if (sqlite3_prepare_v2(db_, sql_state, -1, &stmt_state, nullptr) == SQLITE_OK) {
        std::string val = std::to_string(log_offset);
        sqlite3_bind_text(stmt_state, 1, val.c_str(), -1, SQLITE_TRANSIENT);
