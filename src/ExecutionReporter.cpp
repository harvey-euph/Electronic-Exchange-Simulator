...
void send_response(SHMRingBuffer* ring,
                  ...
                  RejectCode reject_code)
{
    ...
    ring->enqueue(&resp, sizeof(OrderResponseT));
}
} // namespace

ClientExecutionReporter::ClientExecutionReporter(SHMRingBuffer* ring): m_ring(ring) {}
...
{
    if (!req) return;
    send_response(m_ring, ExecType_New, req->order_id(), req->client_id(), req->exec_id(), req->symbol_id(), req->side(), req->p(), req->q(), RejectCode_None);
}
...
void ClientExecutionReporter::onCancelled(const OrderRequest* req)
{
    ...
    send_response(m_ring, ExecType_Cancelled, req->order_id(), req->client_id(), req->exec_id(), req->symbol_id(), req->side(), req->p(), req->q(), RejectCode_None);
}

void ClientExecutionReporter::onModified(const OrderRequest* req)
{
    if (!req) return;
    send_response(m_ring, ExecType_Replaced, req->order_id(), req->client_id(), req->exec_id(), req->symbol_id(), req->side(), req->p(), req->q(), RejectCode_None);
}

void ClientExecutionReporter::onReject(const OrderRequest* req, RejectCode code)
{
    if (!req) return;
    send_response(m_ring, ExecType_Cancelled, req->order_id(), req->client_id(), req->exec_id(), req->symbol_id(), req->side(), req->p(), req->q(), code);
}
                                     ...
                                     uint64_t qty_fill)
{
    ...
    send_response(m_ring, 
                  taker->qty_remaining == 0 ? ExecType_Fill : ExecType_PartialFill,
                  taker->order_id, taker->client_id, rand_exec_id, taker->symbol_id, taker_side, price, qty_fill, RejectCode_None);
    ...
    send_response(m_ring,
                  ...
                  maker->order_id, maker->client_id, rand_exec_id, maker->symbol_id, maker_side, price, qty_fill, RejectCode_None);
}
