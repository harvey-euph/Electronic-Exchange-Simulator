...
OrderBook::OrderBook(
    ...
    , order_pool_("order_pool", 1000000)
{
    if (min_step <= 0) {
...
Order* OrderBook::createOrder(const OrderRequestT* req)
{
...
void OrderBook::processRequest(OrderRequestT* req)
{
    if (!req) {
        return;
    }
    ...
    case OrderAction_Modify:
        if (req->p && price_invalid(req->p)) {
            sendResponse(ExecType_Rejected, req->order_id, req->client_id, req->exec_id, req->side, req->p, req->q, RejectCode_PriceInvalid);
            break;
        }
                              ...
                              RejectCode reject_code)
{
    ...
    OrderResponseT* resp = resp_pool_.allocate();
    if (!resp) return;
    ...
    uint32_t index = resp_pool_.get_index(resp);
    response_ring_->enqueue(&index, sizeof(uint32_t));
}
